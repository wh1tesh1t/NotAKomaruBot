...
from komaru.utils.decorators import stop_here
...
@Client.on_callback_query(filters.regex(f"^{CMD_LIST}$"))
@use_chat_lang
...
@Client.on_message(filters.command(["help", "start -h"]) & filters.private)
@use_chat_lang
@stop_here
...
@Client.on_message(filters.command(["help", "start -h"]))
@use_chat_lang
@stop_here
...
async def get_category(c: Client, m: CallbackQuery, s: Strings):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(s("general_back_btn"), callback_data=f"{CMD_LIST}")]]
    )
